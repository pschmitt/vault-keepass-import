...
def get_entry_name(entry):
    for path_choice in [
        ...
        if value:
            if path_choice == "UUID":
                return "<UUID:{}>".format(
                    binascii.hexlify(base64.b64decode(value)).decode()
                )
            else:
                return value
...
def export_entries_from_group(
    xmldata, group, parent_name=None, force_lowercase=False
):
    group_name = get_group_name(group)
    path = "{}{}".format(
        parent_name if parent_name else "", group_name if group_name else ""
    )
...
def export_entries(
    filename, password, keyfile=None, force_lowercase=False, skip_root=False
):
...
def reset_vault_backend(vault_url, vault_token, vault_backend, ssl_verify=True):
...
def get_next_similar_entry_index(
    vault_url, vault_token, entry_name, ssl_verify=True
):
    return (
        len(
            find_similar_entries(vault_url, vault_token, entry_name, ssl_verify)
        )
        + 1
    )
